 ...
 *   => §17 ledger is NOT deployed on staging: it is ABSENT, not partially passed.
 *
 * Canonical output taxonomy (QA ruling 2026-09-17, binding):
 *   PASS                                harness/setup/cleanup/inventory assertions only
 *   NEUTRAL (coincidental-equal-outcome) a literal repeat happens to return identical
 *                                       status+body via business behavior (double publish,
 *                                       re-ack 200, patch-same-value) - NOT a §17 pass
 *   EXPECTED-FAIL-PENDING-IMPLEMENTATION a §17 claim/replay/409 gap, expected until the
 *                                       ledger lands (default --expect pending)
 *   UNTESTED                            shared-state routes the harness must not touch
 *                                       (whitelist family: no delete route)
 *   FAIL                                harness error (or a real assertion failure under
 *                                       --expect implemented, the post-ledger hard gate)
 *
...
const verdict = (ok, pendingReason) =>
  EXPECT === 'implemented' ? (ok ? 'PASS' : 'FAIL') : (ok ? 'NEUTRAL' : 'EXPECTED-FAIL-PENDING-IMPLEMENTATION');
...
async function triple(group, label, make, { divergentNote } = {}) {
  ...
  rec(group, `${label} replay`, verdict(replayOk, PENDING), { r1: r1.status, r2: r2.status, bodiesEqual: JSON.stringify(r1.json) === JSON.stringify(r2.json), ...(EXPECT === 'pending' ? (replayOk ? { interpretation: 'coincidental-equal-outcome: business behavior, NOT §17 ledger replay' } : { reason: PENDING }) : {}) });
...
async function main() {
  ...
  try {
    ...
    for (const [route, mk] of [
      ...
      if (!IS_LOCAL) { rec('whitelist', `${route} claim/replay/409`, 'UNTESTED', { reason: 'shared-state route: no delete route — whitelist rows cannot be cleaned on shared staging; local-memory runs only' }); continue; }
...
function finish(invOk) {
  ...
  const harnessPass = evidence.filter((e) => e.status === 'PASS');
  const neutral = evidence.filter((e) => e.status === 'NEUTRAL');
  const pending = evidence.filter((e) => e.status === 'EXPECTED-FAIL-PENDING-IMPLEMENTATION');
  const untested = evidence.filter((e) => e.status === 'UNTESTED');
  const summary = { runId: RUN_ID, base: BASE, expect: EXPECT, checks: evidence.length, harnessPass: harnessPass.length, neutralCoincidentalEqualOutcome: neutral.length, expectedFailPendingImplementation: pending.length, untestedSharedState: untested.length, harnessErrors: hardFails.length, inventoryAligned: invOk, throttled, reading: '§17 is ABSENT, not partially passed: every claim/replay/409 row is an expected failure until the ledger lands' };
