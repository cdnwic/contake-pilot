 ...
 *
 * Black-box scope (honest labeling, QA taxonomy):
 *  PROVEN here (job-construction + tenant isolation, live staging, sandbox providers):
...
async function main() {
  ...
  try {
    ...
    rec('construction', 'idempotencyKey shaped eventId+changeRequestId+kind', typeof job.idempotencyKey === 'string' && job.idempotencyKey.includes(A.eventId), { idempotencyKey: job.idempotencyKey });
    rec('construction', 'batchWindowSec = 60 (ND-3 recipient batching)', job.batchWindowSec === 60, { batchWindowSec: job.batchWindowSec });
    ...
    rec('split', `external targets exist for the subscriber-channel group (sandbox fanout): ${external.length}`, external.length > 0, { external: external.slice(0, 3), total: targets.length });
    ...
    rec('isolation', 'org B admin reading org A event graph -> 404 (no cross-tenant existence leak)', crossEvent.status === 404, { httpStatus: crossEvent.status });
    const crossJobs = await call('GET', `/v1/notifications?eventId=${A.eventId}`, { token: tokB });
    rec('isolation', 'org B admin reading org A notification jobs -> 404', crossJobs.status === 404, { httpStatus: crossJobs.status });
    const aJobEvents = new Set(A.jobs.map((j) => j.eventId));
    const bJobEvents = new Set(B.jobs.map((j) => j.eventId));
    rec('isolation', 'org A jobs reference only org A event; org B jobs only org B event (disjoint)', aJobEvents.size === 1 && aJobEvents.has(A.eventId) && bJobEvents.size === 1 && bJobEvents.has(B.eventId), {});
    const bIds = new Set(B.jobs.map((j) => j.id));
    rec('isolation', 'job id sets disjoint across tenants', A.jobs.every((j) => !bIds.has(j.id)), {});

    // --- observability gap, labeled ---
    rec('surface', 'NOTE: DispatchRecord lifecycle (sent/failed/attempts/provider receipts) has NO HTTP surface; sandbox providers are log-only on staging', 'NOTE', { gap: 'provider-level lifecycle evidence needs a read-only deliveries endpoint or server-log capture - flagged to TL, not a test failure', adapters: 'twilio/whatsapp-cloud/webpush sandbox adapters never invent creds; nothing leaves the process' });
  } finally {
  ...
  const summary = { runId: RUN_ID, base: BASE, checks: evidence.length, passed: evidence.length - fails.length - notes.length, notes: notes.length, failed: fails.length, throttled };
