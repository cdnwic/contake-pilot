...
export function buildApp(repo: GraphRepository, auth: AuthService): FastifyInstance {
  ...
  app.post('/v1/webhooks/inbound', async (req, reply) => {
    ...
    // G2 (carried onto the post-merge tree): every remaining STOP path below
    // ALSO flows through the dispatcher's existing handleInboundStop (ND-5):
    // modelled channel -> optedOut=true; unmodelled staff number -> suppression
    // in the shared DispatchStateStore, blocking subsequent sends by send-side
    // dispatchers sharing the store. The v1.20.2 durable consent/suppression
    // machinery and channel.optout audit taxonomy below are unchanged.
