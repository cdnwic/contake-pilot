...
import { Pool } from 'pg';
import { backupUsers, migrateUsersPhone, parseBackupEnv, parseBackupKeyId, parseBackupMacKey, restoreUsers } from '../dist/services/phone-migration.js';
...
try {
  if (args.backup) {
    ...
    let n;
    try {
      n = await publishBackupFile(args.backup, lines, { overwrite: args.overwriteBackup, auth });
    } catch (e) {
      if (e && e.code === 'EEXIST') {
        console.error(`FATAL: backup file already exists (no-clobber): ${args.backup} (pass --overwrite-backup to replace it deliberately)`);
        process.exit(64);
      }
      throw e;
    }
    const head = JSON.parse(lines[0]);
    console.log(`backup: ${n} users rows + index state + manifest + MAC -> ${args.backup} (0600, fsync, ${args.overwriteBackup ? 'atomic replace' : 'atomic no-clobber link'}, temp validated + readback verified, dir fsynced); backupId=${head.backupId}`);
  }
  if (args.restore) {
    const artifactLines = (await readFile(args.restore, 'utf8')).split('\n').filter(Boolean);
    const peek = JSON.parse(artifactLines[0]);
    console.log(`restore: applying backupId=${peek.backupId} createdAt=${peek.createdAt} env=${peek.env} keyId=${peek.keyId} - REPLAY/FRESHNESS GATE: operator must have approved THIS backup for THIS deployment/database before proceeding`);
