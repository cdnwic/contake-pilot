...
const STRONG = '3f8a9c1e5b7d20486f9a1c3e5b7d90124f6a8c0e2b4d6f8a0c1e3b5d7f9a0b1c'; // synthetic, canonical 64-hex (boot-config v2 shape policy)
...
describe('auth secret resolution (fail closed)', () => {
  ...
  it('REFUSES a weak (<min length) managed env secret', () => {
    ...
    expect(() => resolveAuthSecret(undefined)).toThrow(/weak|shorter/i);
  });
...
describe('deployed-boot gate', () => {
  ...
  it('refuses a missing or weak managed secret', () => {
    ...
    expect(() => assertDeployedBoot()).toThrow(/weak|shorter/i);
  });
